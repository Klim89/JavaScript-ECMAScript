...
class Dog extends Animal {
  ...
  fetch() {
    console.log(
      `Dog of breed ${this.breed} with name ${this.name} has brought a ball`
    );
  }
}
...
// Создать класс "Калькулятор" с приватными полями "последний результат"
// и "текущее значение". Класс должен иметь публичные методы "сложить",
// "вычесть", "умножить" и "разделить", которые будут изменять значение текущего
// значения в соответствии с выбранной операцией. Также класс должен иметь
// статическое поле "точность", которое будет задавать количество знаков после
// запятой при выводе результата.
...
// Также класс должен иметь статическое поле "максимальный возраст", которое будет
...
class User {
  ...
  setAge(value) {
    if (value > User.#maxAge) {
      console.error(
        "Превышен максимально допустимы возраст, пришлите копию паспорта"
      );
    } else {
      this.#age = value;
    }
